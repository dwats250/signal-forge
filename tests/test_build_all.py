...
class BuildSiteTests(unittest.TestCase):
  ...
  "data": {
    "WTI": {"week_chg": 10.0}
            ...
            self.assertIn("Drift</div>", index_html)
            ...
            self.assertIn('<div class="command-label">Drift</div>', index_html)
            self.assertRegex(index_html, r'<div class="command-value">(Stable|Emerging|Building)</div>')
            ...
            self.assertLess(index_html.index("Invalidation</div>"), index_html.index("Drift</div>"))
            self.assertLess(index_html.index("Drift</div>"), index_html.index("Posture</div>"))
